...
def get_json(url:str, **kwargs)->Any:
    """
    Returns a get request in json format
    Prints information on the request error if requests can't go through
    """    
    for _ in range(2):
        ...
        if status == 429:
            ...
            print(f"Request failed with status {status} for params:\n{kwargs}")
            break
...
def wide_data(df:pd.DataFrame)->pd.DataFrame:
    """
    Remove all columns containing NaN values
    """
    columns_to_drop = [
        col for col in df.columns 
        if df.loc[df[col].isna()].shape[0] > 0
    ]
    return df.drop(columns=columns_to_drop)

def narrow_data(df:pd.DataFrame)->pd.DataFrame:
    """
    Remove all rows containing NaN values
    """
    return df.dropna()

def read_or_extract(extract_method:Callable, save_file:str, is_save:bool=False, dtype_dict:dict={}, **kwargs)->pd.DataFrame:
        
    load_dotenv()
    RAW_DATA_FOLDER = Path(os.getenv("RAW_DATA_FOLDER")) # type: ignore
    
    try:
        df = pd.read_csv(RAW_DATA_FOLDER/save_file, dtype=dtype_dict)

    except FileNotFoundError:
        df: pd.DataFrame = extract_method(**kwargs)
        
        if is_save:
            df.to_csv(RAW_DATA_FOLDER/save_file, index=False)
